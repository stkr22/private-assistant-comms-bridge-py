...
This is a **voice assistant communication bridge** for smart home systems, consisting of:

1. **WebSocket Server** (`app/main.py`): FastAPI-based server that processes audio streams from clients
2. **Audio Client** (`examples/client.py`): PyAudio-based client that captures/plays audio via WebSocket
3. **MQTT Integration**: Bridges voice commands to assistant systems via MQTT messaging
4. **Wake Word Detection**: OpenWakeWord integration with configurable models (hey_loona, hey_nohvah, hey_nova)
5. **Voice Activity Detection**: Silero VAD for accurate speech detection

### Key Performance Considerations

**Latency Issues** (primary concern):
- Audio streaming uses continuous buffering that may accumulate over time
- Client sends audio chunks every 0.01s via WebSocket (`examples/client.py:108,117`)
- Server processes audio in real-time but may experience memory/buffer buildup
- External API calls to STT/TTS services add variable latency
- MQTT message queuing can introduce delays

**Memory Management**:
- Audio buffers in `AudioProcessor` class may not clean up properly
